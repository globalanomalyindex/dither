...
| DTH-002 | Privacy and product claim | P1 | Confirmed | The README claims no dependencies, while `index.html` requests Archivo and JetBrains Mono from Google Fonts at runtime. | Baseline README and `index.html` head | Entry screen makes no third-party runtime requests and documentation states the actual dependency model. | Open |
| DTH-003 | Release confidence | P1 | Confirmed | The baseline has no automated status checks. | GitHub baseline status and workflow queries | Required CI jobs run on every pull request and block release when failing. | In progress |
...
| DTH-010 | Documentation | P2 | Confirmed | The baseline README omits browser support, privacy, limits, architecture, testing, contribution path, provenance, and deployment. | Baseline README | A reviewer can run, inspect, test, and evaluate the project without private context. | In progress |
...
| DTH-014 | Asset provenance | P2 | Confirmed | The repository does not provide one ledger for fonts, icons, samples, screenshots, and generated assets. | Baseline documentation review | Every non-original asset has source, license, purpose, and modification status. | In progress |
