...
| DTH-006 | Accessibility and focus | P1 | High | No coherent focus-visible system is evident for the control-dense interface. | Stylesheet review, entry tab regression, and pending full keyboard audit | Every interactive control has visible focus, logical order, and reliable restoration after dialogs and modes. | Entry tab keyboard contract resolved; full focus system remains open for Plan 6 |
...
| DTH-016 | Accessibility semantics | P1 | Confirmed | The header declared a `tablist`, but Dither, Grain, and Paint were ordinary buttons without required tab roles, selection state, controlled panels, or arrow-key movement. | Failing axe `aria-required-children` result; direct Playwright tab regression | The header implements a complete tab contract with roles, `aria-selected`, `aria-controls`, labeled panels, roving focus, arrow keys, Home, and End. | Resolved on Plan 0 branch |
